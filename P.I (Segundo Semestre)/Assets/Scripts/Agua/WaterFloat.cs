...
public class WaterFloat : MonoBehaviour
{
    ...
    {
        //Fator de Scale e posical da Scale local
        ...
        //pegar todas as posições de entradas do plano
        ...
        //Pegar a maxima distance de um Edge e computar a maxima distancia
        ...
        //peso da onda
        var height = Mesh.vertices[index(p1.x, p1.z)].y * (max - Vector3.Distance(p1, localPos)) + Mesh.vertices[index(p2.x, p2.z)].y * (max - Vector3.Distance(p2, localPos))
            + Mesh.vertices[index(p3.x, p3.z)].y * (max - Vector3.Distance(p3, localPos)) + Mesh.vertices[index(p4.x, p4.z)].y * (max - Vector3.Distance(p4, localPos));
    ...
    private Vector3[] GenerateVerts()
    {
        ...
        //Distribuição dos verteces igualmente
    ...
    private int[] GeneraTries()
    {
        var tries = new int[Mesh.vertices.Length * 6];

        //Dois triangulos por um quadrado
        for (int x = 0; x < Dimensions; x++)
        {
            for (int z = 0; z < Dimensions; z++)
            {
    ...
    //Gerador de sombras e ondas e arrumando a textura.
